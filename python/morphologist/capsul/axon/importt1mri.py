...
class ImportT1MRI(Process):
    def __init__(self, **kwargs):
        ...
        self.add_trait('input', File(allowed_extensions=['.nii.gz', '.svs', '.vms', '.vmu', '.ndpi', '.scn', '.svslide', '.bif', '.nii', '.jpg', '.gif', '.png', '.mng', '.bmp', '.pbm', '.pgm', '.ppm', '.xbm', '.xpm', '.tiff', '.tif', '.ima', '.dim', '.vimg', '.vinfo', '.vhdr', '.img', '.hdr', '.v', '.i', '.mnc', '.mnc.gz', '']))
        self.add_trait('output', File(allowed_extensions=['.nii.gz', '.nii', '.ima', '.dim'], output=True))
