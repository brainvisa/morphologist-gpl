...
class TalairachTransformationFromNormalization(Process):
    def __init__(self, **kwargs):
        ...
        self.add_field('normalization_transformation', File, read=True, allowed_extensions=['.trm'])
        self.add_field('Talairach_transform', File, write=True, allowed_extensions=['.trm'])
        self.add_field('commissure_coordinates', File, write=True, allowed_extensions=['.APC'], optional=True)
        self.add_field('t1mri', File, read=True, allowed_extensions=['.nii.gz', '.svs', '.bmp', '.dcm', '', '.i', '.v', '.fdf', '.mgh', '.mgz', '.gif', '.ima', '.dim', '.ndpi', '.vms', '.vmu', '.jpg', '.scn', '.mnc', '.nii', '.pbm', '.pgm', '.png', '.ppm', '.img', '.hdr', '.svslide', '.tiff', '.tif', '.vimg', '.vinfo', '.vhdr', '.bif', '.xbm', '.xpm', '.czi', '.mnc.gz'], optional=True)
