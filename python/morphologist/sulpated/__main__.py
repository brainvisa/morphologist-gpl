...
parser = ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
    description='Sulcal Patterns Editor: edit patterns labels for a brainvisa '
    'database.\n'
    ...
    'If the config file contains the sulci and input database locations, then '
    'the sulci and input databases commandline parameters (-s/-i) are not '
    'needed.\n'
    ...
    '        "graph_vsersion": "3.1"\n'
    '    }\n'
    '  - output_database_filter: optional dict with brainvisa filter values, '
    'like for database_filter, but applies to output sulci graphs, in '
    'addition to database_filter (and higher priority). Having an output '
    'filter allows to write output data with different atteributes, like '
    'sulci_recognition_session or graph_version for instance.\n'
    '  - region: optional string, specifies the working region name. ex:\n'
    '    "region": "orbito_frontal"\n'
    '  - sulci_database: optional string, specifies the location of the sulci '
    'database (read/write to allow saving modified labeled sulci graphs)\n'
    ...
    'need to use this option. Forcing the value to "true" is riskier, but '
    'anyway locks will be really checked before saving any data.\n\n'
    'Files, conflicts and backup files:\n\n'
    'Output pattern files are written in the output directory.\n'
    'Output sulci graphs are written in the "sulci" database. If an earlier '
    'save of the same data is present, il will be overwritten. Use the '
    '"input" database (-i) to avoid this.\n'
...
parser.add_argument('-o', '--output',
                    help='output data location [default: current directory]')
