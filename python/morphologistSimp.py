...
class SimplifiedMorphologist(Process):
	...
	add_trait(self,'perform_segmentation',Bool(True))
        ...
        add_trait(self,'commissure_coordinates',File(output=True))
          ##SPM Normalization
        add_trait(self,'anatomical_template', File(output=False,hidden=True))
        add_trait(self,'job_file',File(output=True,hidden=True))
        add_trait(self,'transformations_information',File(output=True,hidden=True))
        add_trait(self,'normalized_t1mri',File(output=True,hidden=True))
        add_trait(self,'talairach_MNI_transform',File(output=True,hidden=True))
        add_trait(self,'source_referential',File(output=False,hidden=True))
        add_trait(self,'normalized_referential',File(output=False,hidden=True))
        add_trait(self,'tal_to_normalized_transform',File(output=False,hidden=True))
        #Bias Correction
	...
	self.set_viewer( 't1mri_nobias', 'soma.gui.pipeline.viewer.show_t1mri_nobias.ShowT1mriNobias',mri='t1mri',mask='t1mri_nobias')
        add_trait(self,'hfiltered',File(output=True,hidden=True))
        add_trait(self,'white_ridges',File(output=True,hidden=True))
	#self.set_viewer( 'white_ridges', 'soma.gui.viewer.show_white_ridges.ShowWhiteRidges',t1mri=self.t1mri,palette="random",mode="linear_on_defined",rate=0.3,wintype="Axial")
        add_trait(self,'variance',File(output=True,hidden=True))
        add_trait(self,'edges',File(output=True,hidden=True))
        add_trait(self,'field',File(output=True,hidden=True))
        add_trait(self,'meancurvature',File(output=True,hidden=True)) 
	      
        #Histogram Analysis
        add_trait(self,'histo',File(output=True,hidden=True))
	...
	self.set_viewer('split_brain', 'soma.gui.pipeline.viewer.show_label_image.ShowLabelImage',image='t1mri_nobias',mask='split_brain')
        add_trait(self,'split_template',File(output=False,hidden=True))
        #Talairach Transformation      
        add_trait(self,'talairach_ACPC_transform',File(output=True,hidden=True))
	...
	add_trait(self,'perform_meshes_and_graphs',Bool(True))
        #Grey/White Surface           
        add_trait(self,'left_hemi_cortex',File(output=True,hidden=True))
	...
	self.set_viewer('left_white_mesh', 'soma.gui.pipeline.viewer.show_white_matter.ShowWhiteMatter',white_mesh='left_white_mesh',side='left')  
        add_trait(self,'right_hemi_cortex',File(output=True,hidden=True))
	...
	self.set_viewer('right_pial_mesh', 'soma.gui.pipeline.viewer.show_hemi.ShowHemi',hemi_mesh='right_pial_mesh',side='right',mri_corrected='t1mri_nobias')
        ...
        add_trait(self,'left_skeleton',File(output=True,hidden=True))
        add_trait(self,'left_roots',File(output=True,hidden=True))
        add_trait(self,'left_sulci_voronoi',File(output=True,hidden=True))
        add_trait(self,'left_cortex_mid_interface',File(output=True,hidden=True))
          ##Right
        add_trait(self,'right_graph',File(output=True))
        add_trait(self,'right_skeleton',File(output=True,hidden=True))
        add_trait(self,'right_roots',File(output=True,hidden=True))
        add_trait(self,'right_sulci_voronoi',File(output=True,hidden=True))
        add_trait(self,'right_cortex_mid_interface',File(output=True,hidden=True))
        #Sulci Recognition
        add_trait(self,'labels_translation_map',File(output=False,hidden=True))
	
	add_trait(self,'perform_sulci_SPAM_recognition',Bool(False))
          ##Left
        add_trait(self,'left_labelled_graph',File(output=True))         
        add_trait(self,'left_posterior_probabilities',File(output=True,hidden=True))
        add_trait(self,'left_labels_priors',File(output=False,hidden=True))
            ###Global
            #show descriptive model
        add_trait(self,'left_global_model',File(output=False,hidden=True))                     
        add_trait(self,'left_tal_to_global_transform',File(output=True,hidden=True))
        add_trait(self,'left_t1_to_global_transform',File(output=True,hidden=True))
            ###Local
            #show descriptive model
        add_trait(self,'left_local_model',File(output=False,hidden=True))
        add_trait(self,'left_local_referentials',File(output=False,hidden=True))
        add_trait(self,'left_direction_priors',File(output=False,hidden=True))
        add_trait(self,'left_angle_priors',File(output=False,hidden=True))
        add_trait(self,'left_translation_priors',File(output=False,hidden=True))
        add_trait(self,'left_global_to_local_transforms',File(output=True,hidden=True))
          ##Right        
        add_trait(self,'right_labelled_graph',File(output=True))
        add_trait(self,'right_posterior_probabilities',File(output=True,hidden=True))                 
        add_trait(self,'right_labels_priors',File(output=False,hidden=True))
            ###Global            
        add_trait(self,'right_global_model',File(output=False,hidden=True))
        add_trait(self,'right_tal_to_global_transform',File(output=True,hidden=True))       
        add_trait(self,'right_t1_to_global_transform',File(output=True,hidden=True))   
            ###Local
        add_trait(self,'right_local_model',File(output=False,hidden=True))
        add_trait(self,'right_local_referentials',File(output=False,hidden=True))
        add_trait(self,'right_direction_priors',File(output=False,hidden=True))
        add_trait(self,'right_angle_priors',File(output=False,hidden=True))
        add_trait(self,'right_translation_priors',File(output=False,hidden=True))       
        add_trait(self,'right_global_to_local_transforms',File(output=True,hidden=True))  
