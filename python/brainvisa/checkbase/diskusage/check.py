...
def check_disk_usage(directory, get_sizes=True, studies_list=studies_list, users_dir='Users',
    ...
    all_users_list = [each for each in os.listdir(
        users_dir) if os.path.isdir(os.path.join(users_dir, each))]
    all_studies_list = [each for each in os.listdir(
        directory) if os.path.isdir(os.path.join(directory, each))]
    ...
    for user in all_users_list:
        ...
        if user in users_list:
            s = 0
            if get_sizes:
                s = get_size(os.path.join(users_dir, user))
            users_space[user] = s
            if verbose:
                print(user, users_space[user],
                      'identified', time.time() - start_time)
        else:
            if not process_undeclared:
                continue
            s = 0
            if get_sizes:
                s = get_size(os.path.join(users_dir, user))
            other_users[user] = s
            if verbose:
                print(user, other_users[user],
                      'undeclared', time.time() - start_time)
    ...
    for study in all_studies_list:
        ...
        if study in studies_list:
            s = 0
            if get_sizes:
                s = get_size(os.path.join(directory, study))
            studies_space[study] = s
            if verbose:
                print(study, studies_space[study],
                      'identified', time.time() - start_time)
        else:
            if not process_undeclared:
                continue
            ...
            if verbose:
                print(study, other_studies[study],
                      'undeclared', time.time() - start_time)
