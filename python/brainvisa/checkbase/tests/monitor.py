...
def process_module(c):
    ...
    for each in getmembers(c, isfunction):
        ...
        # : <div>%s</div><br/>'%(each[0], doc)
        functions_html += '<b>%s</b><br/>' % each[0]
        ...
        if not each_class[0] in read_modules:
            ...
            for each in getmembers(each_class[1], ismethod):
                ...
                if doc:
                    doc = '<br/>'.join(doc.split('\n'))
                # : <div>%s</div><br/>'%(each[0], doc)
                classes_html += '&nbsp;&nbsp;<b>%s</b><br/>' % each[0]
            classes_html += '<br/>'

    # g.generate_from_template(template_pathname, conversion_hashtable)
    html += functions_html + '<br>' + classes_html
...
def update(ev):
    ...
    import math
    p = pdf.PDFReportPrinter('/tmp/report_tmp.pdf', html=html,
                             doc_height=math.exp(-1/float(nb_pages)) * 70.0 / 0.24)
    p.print_()
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        print(
            "usage : monitor.py checkbase_directory template_file html_file [loop]")
        exit(0)
    cbdir = sys.argv[1]
