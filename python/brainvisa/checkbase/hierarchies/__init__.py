...
def detect_hierarchy(directory, returnvotes=False, maxvote=50):
    ...
    for each in items:
        ...
        if os.path.isdir(os.path.join(directory, each)):
            fs_key_items = set(['surf', 'stats', 'src', 'touch',
                                'label', 'bem', 'scripts', 'tmp', 'trash', 'mri'])
            directories = _get_directories(os.path.join(
                directory, each), threshold=len(fs_key_items))
            ...
            for subject in subjects:
                directories.extend(_get_directories(
                    os.path.join(directory, each, subject)))
                ...
                if each_dir in ['t1mri']:
                    ...
                    for e in g:
                        ...
                        subdirectories.extend(
                            _get_directories(e, fullpath=True))
                    subdir2 = []
