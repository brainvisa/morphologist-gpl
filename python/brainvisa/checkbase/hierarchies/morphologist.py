    ...
    def compute_volumes(self):
        ...
        for subject in self.get_flat_subjects():
            ...
            if set(spm_wc_vols).issubset(set(self.existingfiles[0][subject].keys())):
                volumes = get_volumes(
                    *[getfilepath(each, self.existingfiles[0][subject][each]) for each in spm_wc_vols])
            ...
            for key in ['left_greywhite', 'right_greywhite']:
                if key in self.existingfiles[0][subject]:
                    ...
                    for k, v in {'grey': 100., 'white': 200.}.items():
                        self.volumes[subject]['%s_%s' % (side, k)] = pixelsOfValue(
                            data, v) * voxel_size
