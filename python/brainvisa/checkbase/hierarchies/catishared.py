...
keyitems = ['t1natnii', 't1geonii', 't1natdcm', 't1geodcm', 'split_brain', 'brain_mask',
            'snapshot_split', 'action_3dt1', 'spm8_nobias', 'spm8_grey', 'spm8_white', 'spm8_csf']


class CATISharedCheckbase(Checkbase):
    def __init__(self, directory):
        from brainvisa.checkbase.hierarchies import catishared as cs
    ...
    def get_subject_missing_files(self, subject, keyitems=None):
        from brainvisa.checkbase.hierarchies import catishared as cati
        if not keyitems:
            keyitems = cati.keyitems
    ...
    def get_subject_files(self, subject, subdir='*'):
        ...
        import os
        subject_dirs = []
        subdirs = [e for e in glob(os.path.join(
            self.directory, subdir)) if os.path.isdir(e)]
        for s in subdirs:
